...
DateTimeUtil.getDateTime = function(dat,num,type){
	...
	var yearstr = parseInt(mydate.getYear().toString()) + 1900;
...
*船舶是否冻结
*/
function showFreezeMessage(flag) {
	if( flag != constant_freeze_not)
		alert(mssage_cb_freeze_yes); 
}

/**
*根据代码返回现实值
*/
function showYhqWz(key) {
	var result = "22";
	if(key == '11') {
		result =  message_left_up;
	} else if(key == '12') {
		result =  message_left_down;
	} else if(key == '21') {
		result =  message_right_up;
	} else if(key == '22'){
		result =  message_right_down;
	}
	return result;
}
	
/**将字符串转换成json对象*/
function convertDataToJson(condition) {
	var data = {};
	var cons = condition.split('&');
	for(var i = 0; i < cons.length; i++) {
		var keyValue = cons[i].split('=');
...
var swfLoader = function(){
	...
	return{
		...
		loadCzDdyl:function(setting){
	        ...
	        attributes.name = setting.id;
	        attributes.align = "middle";
	        swfobject.embedSWF(
	       		setting.movie, setting.containerId, 
	            "100%", "100%", 
	            swfVersionStr,
	            setting.xiSwfUrlStr,// To use express install, set to playerProductInstall.swf, otherwise the empty string.  
	            flashvars, params, attributes);
	        // JavaScript enabled so display the flashContent div in case it is not replaced with a swf object.
	        swfobject.createCSS("#" + setting.containerId, "display:block;text-align:left;");
	        
	        var flexObject = document.getElementById(setting.id)
	        if(setting.callback){
	        	setting.callback(flexObject);
	        }
	        return flexObject;
		}
	};
